        ...
        <div className="mb-10 bg-zinc-900/50 border border-white/5 rounded-2xl p-6 flex flex-col md:flex-row items-center justify-between gap-6">
          ...
          <div className="flex-1 w-full md:max-w-md">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-zinc-300 text-sm font-medium">
                <strong className="text-white">
                  {Math.round((stats.distribution.positive / (stats.distribution.total || 1)) * 100)}%
                </strong>{" "}
                aprovam esta obra
              </span>
            </div>
            <div className="h-2 bg-zinc-800 rounded-full overflow-hidden flex">
              <div
                style={{ width: `${(stats.distribution.positive / (stats.distribution.total || 1)) * 100}%` }}
            ...
            {isFocusedGeneral && (
              <div className="flex justify-between items-center px-4 pb-4">
                ...
                <div className="flex gap-2">
                  <button onClick={() => setIsFocusedGeneral(false)} className="px-4 py-2 text-sm font-bold text-zinc-400">
                    Cancelar
                  </button>
                  <button
                    onClick={handleSubmitGeneral}
                    disabled={isPostingGeneral || (generalText.trim().length > 0 && generalText.trim().length < 3)}
...
function ReviewItem({
  ...
  return (
    <div className={`group rounded-3xl transition-all duration-500 ${isElite ? `p-8 bg-zinc-900 border ring-1 ${getEliteCardStyle(review.levelTitle)}` : "p-2"} w-full`}>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-3 flex-wrap">
              <Link to={`/app/profile/${review.username}`} className={`font-black text-lg transition-colors ${isElite ? "text-zinc-100 hover:text-white" : "text-white"}`}>
                @{review.username}
              </Link>
            ...
            {isOwner && (
              <div className="flex gap-2">
                <button onClick={() => setIsEditing(!isEditing)} className="text-zinc-400 hover:text-white transition-colors" title="Editar">
                  <Edit2 size={16} />
                </button>
                <button onClick={() => onDelete(review.id)} className="text-zinc-400 hover:text-red-500 transition-colors" title="Excluir">
                  <Trash2 size={16} />
                </button>
              </div>
            )}
          ...
          {isEditing ? (
            <div className="bg-zinc-950/50 p-4 rounded-xl border border-white/10 mb-4 animate-in fade-in zoom-in duration-200 w-full">
              <div className="flex gap-2 mb-3">
                {[1, 2, 3, 4, 5].map((star) => (
                  <button key={star} onClick={() => setEditRating(star)}>
                    <Star size={18} className={star <= editRating ? "fill-yellow-500 text-yellow-500" : "fill-zinc-800 text-zinc-700"} />
                  </button>
                ))}
              </div>
              <textarea
              ...
              <div className="flex justify-end gap-2 mt-3">
                <button onClick={() => setIsEditing(false)} className="px-3 py-1.5 text-zinc-400 text-xs font-bold hover:text-white transition-colors">
                  Cancelar
                </button>
                <button onClick={handleSaveReviewEdit} className="bg-violet-600 text-white px-4 py-1.5 rounded-lg text-xs font-bold hover:bg-violet-500 transition-colors">
                  Salvar
                </button>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-2 bg-zinc-900/50 p-4 rounded-xl border border-white/5 shadow-inner mb-4 w-full">
              <div className="flex items-center gap-2">
                {[1, 2, 3, 4, 5].map((star) => (
                  <Star key={star} size={28} className={`${star <= review.rating ? "fill-yellow-400 text-yellow-400" : "fill-zinc-800 text-zinc-700"} drop-shadow-md`} />
                ))}
            ...
            <div className={`leading-relaxed mb-4 whitespace-pre-wrap break-words w-full ${isElite ? "text-xl text-white font-medium" : "text-lg text-zinc-200"}`}>
              {isExpanded || !review.text || review.text.length <= MAX_TEXT_LENGTH ? review.text : <>{review.text.slice(0, MAX_TEXT_LENGTH)}...</>}
              {review.text && review.text.length > MAX_TEXT_LENGTH && (
                <button onClick={() => setIsExpanded(!isExpanded)} className="text-violet-400 hover:text-violet-300 font-bold ml-1 text-sm inline-block">
          ...
          {isReplying && (
            <div className="flex gap-2 mt-4 animate-in slide-in-from-top-2 w-full">
              ...
              />
              <button onClick={handleSubmitReply} className="bg-violet-600 text-white p-2 rounded-xl hover:bg-violet-500 transition-colors">
                <Send size={18} />
              </button>
            </div>
          )}
                    ...
                    <div key={reply.id} className="flex gap-3 group/reply w-full">
                      <div className="w-8 h-8 rounded-full bg-zinc-800 overflow-hidden shrink-0">
                        {reply.userPhoto ? (
                          ...
                          <div className="w-full h-full flex items-center justify-center text-[10px] text-zinc-500">U</div>
                        )}
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between mb-1">
                          <div className="flex items-center gap-2">
                            <Link to={`/app/profile/${reply.username}`} className="font-bold text-zinc-200 text-sm hover:text-violet-400">
                              @{reply.username}
                            </Link>
                            <span className="text-[10px] text-zinc-600">
                              {reply.createdAt ? new Date(reply.createdAt._seconds * 1000).toLocaleDateString() : "agora"}
                            </span>
                            {reply.isEdited && <span className="text-[10px] text-zinc-600 italic">(editado)</span>}
                          </div>
                          {reply.isOwner && (
                            <div className="flex gap-2">
                              <button onClick={() => startEditingReply(reply)} className="text-zinc-400 hover:text-white transition-colors" title="Editar">
                                <Edit2 size={12} />
                              </button>
                              <button onClick={() => onDeleteComment(reply.id)} className="text-zinc-400 hover:text-red-500 transition-colors" title="Excluir">
                                <Trash2 size={12} />
                              </button>
                            </div>
                          )}
                        ...
                        {editingReplyId === reply.id ? (
                          <div className="mt-1 animate-in fade-in zoom-in duration-200 w-full">
                            ...
                            <div className="flex justify-end gap-2">
                              <button onClick={() => setEditingReplyId(null)} className="text-[10px] text-zinc-500 font-bold uppercase hover:text-white">
                                <X size={14} />
                              </button>
                              <button onClick={() => handleSaveReplyEdit(reply.id)} className="text-[10px] text-green-500 font-bold uppercase hover:text-green-400">
                                <Check size={14} />
                              </button>
                            </div>
                          </div>
                        ) : (
                          <p className="text-zinc-300 text-sm break-words">{reply.text}</p>
                        )}
                      </div>
                    </div>
                  ))}
                  <button onClick={() => setShowReplies(false)} className="text-[10px] font-bold text-zinc-600 uppercase hover:text-zinc-400 mt-2">
                    Ocultar
                  </button>
                </div>
              )}
