...
export default function Register() {
  ...
  const { register, loginWithGoogle, completeGoogleRegistration, user } = useAuth();
  ...
  useEffect(() => {
    const pendingGoogleProfile = location.state?.googleCompletion;
    if (!pendingGoogleProfile?.requiresProfile) return;

    setGoogleProfile(pendingGoogleProfile);
    setGoogleNickname(pendingGoogleProfile.suggestedUsername || '');
    setFormData((prev) => ({
      ...prev,
      name: pendingGoogleProfile.name || prev.name,
      email: pendingGoogleProfile.email || prev.email,
    }));
  }, [location.state]);
  ...
  const isGoogleNicknameValid = useMemo(
    () => googleNickname.length >= 3 && NICKNAME_REGEX.test(googleNickname),
    [googleNickname]
  );

  const googleProfileInitial = useMemo(() => {
    const source = googleProfile?.name || googleProfile?.email || 'CineSorte';
  ...
  const handleGoogleRegister = async () => {
    ...
    try {
      ...
      if (result?.requiresProfile) {
        ...
        setGoogleNickname(result.suggestedUsername || '');
        setFormData((prev) => ({
          ...prev,
      ...
      setError(err.message || 'Não foi possível continuar com Google.');
    } finally {
  ...
  const handleGoogleComplete = async (event) => {
    event.preventDefault();
    const normalizedNickname = googleNickname.trim().toLowerCase();

    if (!isGoogleNicknameValid) {
      setError('Nome de usuário inválido. Use letras minúsculas, números ou _.');
      return;
    }
    ...
    try {
      ...
      if (result?.requiresProfile) {
        setGoogleProfile(result);
        setGoogleNickname(result.suggestedUsername || normalizedNickname);
        return;
      }

      navigate(redirectTo);
    } catch (err) {
      setError(err.message || 'Não foi possível finalizar seu cadastro com Google.');
    } finally {
  ...
  return (
    <>
      <AuthShell
        eyebrow={googleProfile ? 'Cadastro com Google' : 'Primeiro acesso'}
        title={googleProfile ? 'Finalize seu cadastro' : 'Criar sua conta'}
        description={
          googleProfile
            ? 'Escolha seu usuário e aceite os termos para liberar sua conta no CineSorte.'
            : 'Leva menos de um minuto para começar a salvar filmes, escrever reviews e conversar com outros perfis.'
        }
        footer={footer}
            ...
            <div className="space-y-2.5">
              ...
              <div className="group relative">
                ...
                <input
                  ...
                  onChange={(event) => setGoogleNickname(event.target.value.toLowerCase().replace(/\s/g, ''))}
                />
                {isGoogleNicknameValid && <CheckCircle2 size={17} className="absolute right-4 top-1/2 -translate-y-1/2 text-emerald-300" />}
              </div>
            </div>
