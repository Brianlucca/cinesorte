...
export default function ReviewsList({ reviews, hasMore = false, loadingMore = false, onLoadMore }) {
  ...
  const canGoNext = currentPage < loadedPages - 1 || hasMore;
  const totalPagesLabel = hasMore ? `${loadedPages}+` : loadedPages;
  const pageItems = reviewItems.slice(currentPage * REVIEWS_PER_PAGE, (currentPage + 1) * REVIEWS_PER_PAGE);
  const firstReviewId = reviewItems[0]?.id || null;
  ...
  const goToNextPage = async () => {
    ...
    if (loadedCount > 0) setPage((value) => value + 1);
  };
